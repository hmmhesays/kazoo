...
find_numbers(_Number, _Quanity, _Opts) ->
    %% TODO: given the requestors account number discovery knm_local numbers that are
    %%       available but managed by accendants of the account.
    {'error', 'non_available'}.


-spec is_number_billable(knm_phone_number:knm_number()) -> 'false'.
...
%% @end
%%--------------------------------------------------------------------
-spec acquire_number(knm_phone_number:knm_number()) -> knm_phone_number:knm_number().
...
disconnect_number(Number) ->
    knm_number:change_state(knm_phone_number:number(Number), ?NUMBER_STATE_RELEASED).
