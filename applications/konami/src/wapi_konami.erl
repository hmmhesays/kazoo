...
transferred(API) ->
    case transferred_v(API) of
        'true' -> wh_api:build_message(API, ?TRANSFERRED_HEADERS, ?OPTIONAL_TRANSFERRED_HEADERS);
        'false' -> {'error', <<"API failed validation for transferred">>}
    end.

-spec transferred_v(api_terms()) -> boolean().
transferred_v(API) ->
    wh_api:validate(API, ?TRANSFERRED_HEADERS, ?TRANSFERRED_VALUES, ?TRANSFERRED_TYPES).
...
-spec bind_q(ne_binary(), api_binary(), api(wh_proplist())) -> 'ok'.
...
-spec unbind_q(ne_binary(), api_binary(), api(wh_proplist())) -> 'ok'.
...
publish_transferred(TargetCallId, API) ->
    {'ok', Payload} = wh_api:prepare_api_payload(API, ?TRANSFERRED_VALUES, fun ?MODULE:transferred/1),
    amqp_util:whapps_publish(transferred_routing_key(TargetCallId), Payload).

-spec bind_for_transferred(ne_binary(), ne_binary()) -> 'ok'.
bind_for_transferred(Queue, CallId) ->
    manipulate_queue_bindings(Queue, transferred_routing_key(CallId), fun amqp_util:bind_q_to_whapps/2).

-spec unbind_for_transferred(ne_binary(), ne_binary()) -> 'ok'.
unbind_for_transferred(Queue, CallId) ->
    manipulate_queue_bindings(Queue, transferred_routing_key(CallId), fun amqp_util:unbind_q_from_whapps/2).
