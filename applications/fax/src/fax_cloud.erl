...
get_faxbox_doc(PrinterId) ->
    case kzc_cache:peek(?CACHE_NAME, {'faxbox', PrinterId }) of
        {'ok', _Doc}=OK -> OK;
        {'error', _} ->
...
fetch_faxbox_doc(PrinterId) ->
    ...
    case couch_mgr:get_results(?WH_FAXES_DB, <<"faxbox/cloud">>, ViewOptions) of
        ...
        {'ok', [JObj]} ->
            ...
            kzc_cache:store(?CACHE_NAME, {'faxbox', PrinterId }, FaxBoxDoc, CacheProps),
            {'ok', FaxBoxDoc}
    end.
...
get_printer_oauth_credentials(PrinterId) ->
    case kzc_cache:peek(?CACHE_NAME, {'gcp', PrinterId }) of
        {'ok', _Auth}=OK -> OK;
        {'error', _} ->
...
fetch_printer_oauth_credentials(PrinterId) ->
    case get_faxbox_doc(PrinterId) of
        ...
        {'ok', JObj} ->
            ...
            kzc_cache:store(?CACHE_NAME, {'gcp', PrinterId}, Auth, [{'expires', Expires}]),
            {'ok', Auth}
    end.
