...
lookup(JObj) ->
    ...
    case kzc_cache:fetch(?CACHE_NAME, cache_key(Num)) of
        {'ok', CNAM} ->
...
flush() ->
    kzc_cache:filter_erase(?CACHE_NAME, fun flush_entries/2).
...
fetch_cnam(Number, JObj) ->
    case make_request(Number, JObj) of
        ...
        CNAM ->
            ...
            kzc_cache:store(?CACHE_NAME, cache_key(Number), CNAM, CacheProps),
            CNAM
    end.
