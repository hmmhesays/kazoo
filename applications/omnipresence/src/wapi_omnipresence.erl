...
notify(Prop) when is_list(Prop) ->
    case notify_v(Prop) of
        'true' -> wh_api:build_message(Prop, ?NOTIFY_HEADERS, ?OPTIONAL_NOTIFY_HEADERS);
        'false' -> {'error', "Proplist failed validation for subscription"}
    end;
notify(JObj) -> notify(wh_json:to_proplist(JObj)).

-spec notify_v(api_terms()) -> boolean().
notify_v(Prop) when is_list(Prop) ->
    wh_api:validate(Prop, ?NOTIFY_HEADERS, ?NOTIFY_VALUES, ?NOTIFY_TYPES);
notify_v(JObj) -> notify_v(wh_json:to_proplist(JObj)).
...
search_partial_resp(Prop) when is_list(Prop) ->
    case search_partial_resp_v(Prop) of
        'true' -> wh_api:build_message(Prop, ?SEARCH_PARTIAL_RESP_HEADERS, ?OPTIONAL_SEARCH_PARTIAL_RESP_HEADERS);
        'false' -> {'error', "Proplist failed validation for search_resp"}
    end;
search_partial_resp(JObj) ->
    search_partial_resp(wh_json:to_proplist(JObj)).

-spec search_partial_resp_v(api_terms()) -> boolean().
search_partial_resp_v(Prop) when is_list(Prop) ->
    wh_api:validate(Prop, ?SEARCH_PARTIAL_RESP_HEADERS, ?SEARCH_PARTIAL_RESP_VALUES, ?SEARCH_PARTIAL_RESP_TYPES);
search_partial_resp_v(JObj) ->
    search_partial_resp_v(wh_json:to_proplist(JObj)).
...
publish_search_partial_resp(Queue, Resp, ContentType) ->
    {'ok', Payload} = wh_api:prepare_api_payload(Resp, ?SEARCH_PARTIAL_RESP_VALUES, fun ?MODULE:search_partial_resp/1),
...
-spec unbind_q(ne_binary(), api(atoms()), wh_proplist()) -> 'ok'.
